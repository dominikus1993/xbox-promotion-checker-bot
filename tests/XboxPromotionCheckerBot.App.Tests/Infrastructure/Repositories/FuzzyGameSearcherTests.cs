...
public sealed class FuzzyGameSearcherTests
{
    ...
    public async Task TestWhenGameExists()
    {
        ...
        var subject = await searcher.Filter(new []{ Game.Create("Stellaris Enchanced", new Uri("http://localhost"), new GamePrice(10, 20))}.ToAsyncEnumerable()).ToListAsync();
    ...
    public async Task TestWhenGameExistsAndNotExistsInSecondQuery()
    {
        ...
        var subject = await searcher.Filter(new []{ Game.Create("Stellaris Enchanced", new Uri("http://localhost"), new GamePrice(10, 20)), Game.Create("Cyberpunk 2077 Complete Edition", new Uri("http://localhost"), new GamePrice(10, 20))}.ToAsyncEnumerable()).ToListAsync();
    ...
    public async Task TestWhenGameNotExists()
    {
        ...
        var subject = await searcher.Filter(new []{Game.Create("Cyberpunk 2077 Complete Edition", new Uri("http://localhost"), new GamePrice(10, 20))}.ToAsyncEnumerable()).ToListAsync();
    ...
    public async Task TestParsingWhenGamesAreInGamesThatIWantBuy()
    {
        ...
        var searcher = new GameNameFilter(games);
        Game[] gamesFromSomething = [Game.Create("Cyberpunk 2077", new Uri("http://localhost"), new GamePrice(10, 20)), Game.Create("Stellaris Enchanced", new Uri("http://localhost"), new GamePrice(10, 20)), Game.Create("Assasins Creed", new Uri("http://localhost"), new GamePrice(10, 20)), Game.Create("STORY OF SEASONS: Friends of Mineral Town - Digital Edition", new Uri("http://localhost"), new GamePrice(10, 20))];
